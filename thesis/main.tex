...
\newcommand{\studentfullname}{Olimjon Umurzokov}
\newcommand{\studentemail}{TODO}  % Only relevant for term papers, ignored for bachelor's and master's theses
\newcommand{\studentID}{50334434}

\newcommand{\manuscripttitle}{High-Dimensional Instrumental Variable Quantile Regression under Weak Instruments: A Monte Carlo Study}
\newcommand{\manuscriptrunningtitle}{High-Dimensional IVQR under Weak Instruments}

\newcommand{\manuscriptdate}{2026-09-17}
\newcommand{\formattedmanuscriptdate}{%
	\ifthenelse{\equal{\manuscriptdate}{TODO}}{TODO}{\printdate{\manuscriptdate}}%
}
...
\ifthenelse{%
	...
	\newcommand{\secondmajor}{TODO}
}{% English
	\newcommand{\secondmajor}{\textit{\foreignlanguage{ngerman}{TODO}}}
}

\newcommand{\seminartitle}{TODO}  % Only relevant for term papers, ignored for bachelor’s and master’s theses
\newcommand{\advisorname}{JProf. Claudia Noack Ph.D.}
\newcommand{\advisorgender}{TODO}  % TODO: f, m, or nonbinary; relevant to German output
...
\fancyfoot[C]{\sffamily\small\relscale{0.95}\strut\thepage}
...
\newcommand*{\maketitleEN}{%
	...
	\begin{center}
		...
		\vfill\vfill
		{\ifthenelse{\boolean{manuscriptisbachelor}}{Bachelor's Thesis}{%
		...
		\vfill\vfill%\vfill
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
			Submitted in \printdayoff\formattedmanuscriptdate%
		}{%
			Submitted on \formattedmanuscriptdate%
		}
		by \\[1.5\baselineskip]
...
\newcommand*{\maketitleDE}{%
	...
	\begin{center}
		...
		\vfill\vfill%\vfill
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
			Vorgelegt im \datengerman\printdayoff\formattedmanuscriptdate%
		}{
			Vorgelegt am \datengerman\formattedmanuscriptdate%
		}
...
% Keep continued long-table captions visible without creating duplicate
% Hyperref destinations. The firsthead template remains unchanged and owns
% the table's real anchor; only middle and final continuation heads are local.
\ExplSyntaxOn
	...
	\IfBoolTF \l__tblr_table_no_label_bool
		...
		\IfBoolTF \l__tblrextras_table_title_below_bool
			...
			\begin{NoHyper}
				...
				\caption*{\textbf{\tablename~\thetable.}~\InsertTblrText{caption}~\UseTblrTemplate{conthead-text}{default}}
			\end{NoHyper}
			}
		}
}
...
% Adjust only this Abstract's heading and first quotation paragraph.
\patchcmd{\abstract}{\bfseries \abstractname}
  {\hspace*{0.5cm}\Large\bfseries \abstractname}
  {}{\PackageError{thesis}{Abstract heading patch failed}{}}
\patchcmd{\quotation}{\listparindent = \smalllinespacing}
  {\listparindent = \smalllinespacing\itemindent = \listparindent}
  {}{\PackageError{thesis}{Abstract indentation patch failed}{}}
\begin{abstract}
\normalsize\color{black}
This thesis studies the finite-sample consequences of weakening instrument relevance in high-dimensional instrumental variable quantile regression (IVQR). Building on a preserved DML-IVQR Monte Carlo design, instrument strength is systematically varied through \(\kappa \in \{1, 0.50, 0.25, 0.10\}\) while holding the marginal treatment distribution, structural outcome equation, and control dimension fixed. Three estimators---Oracle-GMM (an infeasible benchmark with known relevant controls), Full-GMM (unregularized estimation with all controls), and DML-IVQR (regularized nuisance estimation with pivotal penalty calibration)---are compared across five quantiles, \(\tau \in \{0.10, 0.25, 0.50, 0.75, 0.90\}\), and sample sizes \(n \in \{500, 1000\}\).

The results show that as \(\kappa\) falls, point-estimation errors (MAE and RMSE) rise sharply, accepted sets expand, and power against false treatment-effect values declines for all estimators, including the oracle benchmark. Coverage is also not always well calibrated in some finite-sample cells, even under strong instrument relevance. DML often improves on Full-GMM but does not uniformly match Oracle-GMM across all design cells. The main conclusion is that Neyman orthogonalization and regularized nuisance estimation can reduce high-dimensional estimation costs, but they cannot replace identifying information lost when instruments become weakly relevant.
\end{abstract}
\endgroup
...
\input{appendix/appendix_numerical_domain}
\input{appendix/appendix_coverage_audit_minimal}
\input{appendix/appendix_reproducibility_note}
