...
This thesis studies the finite-sample consequences of weakening excluded-instrument relevance in high-dimensional instrumental variable quantile regression (IVQR). Building on a preserved DML-IVQR Monte Carlo design, instrument strength is varied through $\kappa\in\{1,0.50,0.25,0.10\}$ while holding the marginal treatment distribution, endogeneity channel, structural outcome equation, and control dimension fixed. Oracle-GMM, Full-GMM, and DML-IVQR-BC are compared across five quantiles and $n\in\{500,1000\}$. As $\kappa$ falls, MAE and RMSE rise, accepted sets expand, and power against false treatment-effect values declines for all estimators, including the oracle benchmark. Coverage is also imperfectly calibrated in some finite-sample cells, even under strong relevance. DML often improves on Full-GMM but does not uniformly match Oracle-GMM. The results show that orthogonalization and regularized nuisance estimation can reduce high-dimensional estimation costs, but they cannot replace identifying information lost when instruments become weakly relevant.
\end{abstract}
\endgroup
