...
\fancyhead[L]{\sffamily\small\relscale{0.95}\strut\studentfullname: \textit{\manuscriptrunningtitle}}
...
\newcommand*{\maketitleEN}{%
	...
	\begin{center}
		...
		\Large
		\null\vfill
		{\fontsize{20pt}{26pt}\bfseries\manuscripttitle\par}
		\vfill\vfill
...
\textbf{This thesis studies the finite-sample consequences of weakening excluded-instrument relevance in high-dimensional instrumental variable quantile regression (IVQR)}. Building on a preserved DML-IVQR Monte Carlo design, instrument strength is systematically varied through $\kappa \in \{1, 0.50, 0.25, 0.10\}$ while holding the marginal treatment distribution, endogeneity channel, structural outcome equation, and control dimension fixed. Three estimators---\textbf{Oracle-GMM} (an infeasible benchmark with known relevant controls), \textbf{Full-GMM} (unregularized estimation with all controls), and \textbf{DML-IVQR-BC} (regularized nuisance estimation with pivotal penalty calibration)---are compared across five quantiles ($\tau \in \{0.10, 0.25, 0.50, 0.75, 0.90\}$) and sample sizes $n \in \{500, 1000\}$.

\textbf{The results show that as $\kappa$ falls, point-estimation errors (MAE and RMSE) rise substantially, accepted sets expand, and power against false treatment-effect values declines for all estimators, including the oracle benchmark}. Coverage is also imperfectly calibrated in some finite-sample cells, even under strong instrument relevance. DML often improves on Full-GMM but does not uniformly match Oracle-GMM across all design cells. \textbf{The main conclusion is that Neyman orthogonalization and regularized nuisance estimation can reduce high-dimensional estimation costs, but they cannot replace identifying information lost when instruments become weakly relevant}.
\end{abstract}
\endgroup
