...
Each design is evaluated using Oracle-GMM, Full-GMM, and DML-IVQR. Oracle-GMM uses the known low-dimensional set of controls relevant to the DGP and serves as an infeasible benchmark. Full-GMM instead uses the complete high-dimensional control set without the DML regularization strategy. DML-IVQR applies the authors' high-dimensional regularized procedure with an orthogonal score. Chapter~3 provides the formal estimator definitions. Combining the 40 designs with the three estimators yields $40\times3=120$ design-estimator cells, each containing 500 replications. For every quantile index, the true parameter used in the Monte Carlo evaluation is the value from Equation~\eqref{eq:dgp-alpha-true}, $\alpha_0(\tau)=1+\Phi^{-1}(\tau)$.

Within a given replication and $(n,\tau,\kappa)$ design, the runner evaluates all three estimators on the same generated sample, thereby holding the simulated data fixed across estimator comparisons. The final DML-IVQR implementation preserves the authors' theoretical Belloni--Chernozhukov pivotal penalty route as implemented, uses fresh pivotal Uniform draws at each relevant penalty call, and does not employ cross-fitting or sample splitting.
...
For a hypothesized structural quantile treatment effect $a$, let $\widehat{g}_N(a)$ denote the estimated IVQR moment vector evaluated at that candidate and let $\widehat{\Sigma}(a)$ denote its estimated covariance matrix. The candidate-specific GMM statistic is
\begin{equation}
  W_N(a)
  =N\widehat{g}_N(a)^{\prime}
    \widehat{\Sigma}(a)^{-1}\widehat{g}_N(a).
  \label{eq:wn-statistic}
\end{equation}
The covariance estimate standardizes the sampling variation of the moments, so smaller values of $W_N(a)$ indicate greater compatibility between the candidate and the sample moments. Under the null that $a$ equals the true structural parameter, the implemented weak-identification-robust procedure uses the asymptotic reference distribution
\begin{equation}
  W_N(a)\overset{d}{\longrightarrow}\chi^2_{\dim(Z)},
  \qquad
  \dim(Z)=2,
  \qquad
...
\end{equation}
This asymptotic cutoff is not an exact finite-sample guarantee; finite-sample calibration is evaluated empirically in Chapter~5. The construction follows the robust test-inversion approach of \textcite{chernozhukov_hansen_2008} and its high-dimensional implementation in \textcite{chen_huang_tien_2021}.

Conceptually, the nominal 95\% confidence set is obtained by retaining candidate values whose null hypotheses are not rejected:
\begin{equation}
  \mathcal{C}_{0.95}(\tau)
  =\left\{a:W_N(a)\leq c_{0.95}\right\}.
  \label{eq:wn-inversion}
\end{equation}
Test inversion does not require this set to be connected; it may in principle contain separated components. The main simulation evaluates the profile on the prespecified primary grid
\begin{equation}
  A_0=[-1,3],
  \qquad h=0.1,
  \qquad
