...
\end{equation}
Because all other instrument components are independent of $d_i^*(\kappa)$, each instrument--index covariance falls as $\kappa$ decreases. The parameter governs relevance but is not a complete measure of IVQR identification. Endogeneity remains fixed because $u_i$ is independent of $z_{1i}$, $z_{2i}$, and $w_i$ while its covariance with $\epsilon_i$ is unchanged:
\begin{equation}
...
\end{equation}
Here $A_0=[-1,3]$ is the fixed primary computational domain, $h=0.1$ is the distance between adjacent grid points, $j$ is the grid-point index, and $a_j$ denotes the $j$-th candidate treatment-effect value. The set $G_0$ contains all 41 candidate values. There are 41 points because the four-unit interval from $-1$ to $3$, evaluated in increments of $0.1$ and including both endpoints, contains $4/0.1+1=41$ grid points. The same grid is used throughout. $A_0$ is a computational domain, not an asserted parameter space, so reaching its boundary establishes neither global boundedness nor unboundedness.
...
\end{equation}
The same definition is used for an expanded diagnostic domain, with its own left and right endpoints. A boundary is accepted when the candidate-specific test does not reject the endpoint of the finite numerical search interval. A high boundary-acceptance rate therefore means that accepted components frequently reach the edge of the examined domain and are not localized inside it. It does not show that the population confidence region is globally unbounded, that every value outside the domain would be accepted, or that identification has failed. Conversely, if boundary acceptance disappears after the domain is widened, the accepted component is numerically localized within the wider range. Persistent boundary acceptance as successively wider finite domains are examined indicates continued lack of numerical localization over those investigated ranges.
...
\end{equation}
The sum runs from $j=1$ to $40$ because the 41 grid points in $G_0$ define 40 adjacent grid intervals $[a_j,a_{j+1}]$. The trapezoidal sum includes separated accepted components without filling rejected gaps and does not interpolate critical-value crossings. It ranges from zero to four. The Monte Carlo mean of $F_r$ is the frequency with which all tested grid points are accepted, not acceptance of the real line.
