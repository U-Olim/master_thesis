...
The intercept retained by the frozen transformed regression is an implementation-specific quantity. Because it enters the transformed regression as an ordinary constant rather than as $\sqrt{\widehat f_i(a)}$ times a constant regressor, $\widehat r_0(a)$ is not the literal sample counterpart of $d_0(a)$ in the augmented population projection of Section~3.2. The exact derivative cancellation established in Section~3.3 applies to the theoretical augmented score. The preserved executable score follows the same density-weighted residualization logic for the slope component but differs in its treatment of the residualization intercept. The Monte Carlo results therefore characterize the preserved executable DML-IVQR implementation rather than a literal sample implementation of every component of the augmented theoretical orthogonal score.

The $\ell_1$ penalties place a cost on large collections of nonzero coefficients. Weak coefficients can consequently be shrunk strongly, often to zero, which can stabilize both nuisance steps in a high-dimensional sample. This regularization also creates shrinkage error; the theoretical orthogonal construction in Section~3.3 is designed to limit its first-order effect on the target moment, as in the general framework of \textcite{chernozhukov_et_al_2018}. The implementation estimates the nuisance parameters and evaluates the score on the same sample; it does not use sample splitting or cross-fitting. General DML procedures often use cross-fitting, whereas this preserved implementation uses same-sample evaluation with its high-dimensional sparsity and rate conditions.
