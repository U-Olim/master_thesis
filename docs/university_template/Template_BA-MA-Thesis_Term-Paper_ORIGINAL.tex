...
% For compiling this document without any changes, a *complete* (and up-to-date) TeX distribution is required. Options include MacTeX, MiKTeX, and TeX Live.

% For MiKTeX, please choose the “Net Installer” to install the complete MiKTeX distribution, rather than the “Basic Installer.”

% Compilation via https://overleaf-en.astro.uni-bonn.de works in general.  Use “Shibboleth login” to log in with your Uni ID. Unfortunately, compiling the entire document may fail. Try compiling a shorted version (e.g., remove the appendices).

% Compilation via https://uni-bonn.sciebo.de/apps/overleaf_nextcloud/launcher/launch also works in general. Here, compiling the entire document from scratch may lead to a time-out. Compiling the document again (and again) can help. So does compiling a shortened version (which can then be extended step by step).
...
\newcommand{\studentfullname}{\AE nn\i\ \'{E}lu\"{i}\.{z} Sn\^{e}\v{z}{\`a}\~{n}\'{a} H\r{a}land-\c{C}a{\l}han\H{o}\u{g}l\"{u}}
\newcommand{\studentemail}{ae.haaland-calhanoglu@example.org}  % Only relevant for term papers, ignored for bachelor’s and master’s theses
...
\newcommand{\manuscriptdate}{2026-07-27}  % In ISO format: yyyy-mm-dd

\newcommand{\manuscriptlanguage}{EN}  % DE for German, otherwise (US) English

\newcommand{\manuscripttype}{TP}  % Term paper by default
% Enable one when writing a thesis:
%\renewcommand{\manuscripttype}{BA-2F}  % Bachelor of Arts, 2-Fach-Modell (double-major program)
%\renewcommand{\manuscripttype}{BSc-ECS}  % Bachelor of Science in Economics and Computer Science
%\renewcommand{\manuscripttype}{BSc-VWL}  % Bachelor of Science in Volkswirtschaftslehre (Economics)
\renewcommand{\manuscripttype}{MSc-Econ}  % Master of Science in Economics

\RequirePackage{ifthen}
% Only relevant for BA-2F: 2nd major in the double-major bachelor’s program
\ifthenelse{%
	\equal{\manuscriptlanguage}{DE}%
}{% German
	\newcommand{\secondmajor}{XXXXXXXXXX}
}{% English
	\newcommand{\secondmajor}{\textit{\foreignlanguage{ngerman}{XXXXXXXXXX}}}
}

\newcommand{\seminartitle}{%
	Exercises in Theoretical and Empirical Microeconomics and Macroeconomics, Using\nolinebreak\ State-of-the-Art Econometric Methods, to\nolinebreak\ Address Societal Issues All Around the World%
}  % Only relevant for term papers, ignored for bachelor’s and master’s theses
\newcommand{\advisorname}{Prof.~Dr. V\ae-Rie Sm\={a}rt}
\newcommand{\advisorgender}{f}  % f for female, m for male, otherwise nonbinary
...
%\documentclass[12pt, a4paper, oneside]{article}
	% In line with the old (2014) guidelines for master’s theses of the Department of Economics at the University of Bonn:
	% https://www.econ.uni-bonn.de/examinations/en/information/master-economics/master-thesis
\documentclass[11pt, a4paper, oneside]{article}
	% 11pt font size
	% In line with the 2026 guidelines for bachelor’s theses of the Department of Economics at the University of Bonn:
	% https://www.econ.uni-bonn.de/examinations/de/informationen/bachelor/bachelorarbeit
	% Page margins are configured below such that approximately the same amount of text fits on a page as per the 2014 guidelines
...
\newboolean{manuscriptisbachelor}
\newboolean{manuscriptismaster}
\ifthenelse{%
	\equal{\manuscripttype}{BA-2F} \OR
	\equal{\manuscripttype}{BSc-ECS} \OR
	\equal{\manuscripttype}{BSc-VWL}%
}{%
	\setboolean{manuscriptisbachelor}{true}%
}{%
	\setboolean{manuscriptisbachelor}{false}%
}
\ifthenelse{%
	\equal{\manuscripttype}{MSc-Econ}%
}{%
	\setboolean{manuscriptismaster}{true}%
}{%
	\setboolean{manuscriptismaster}{false}%
}
\ifthenelse{%
	\equal{\manuscriptlanguage}{DE}%
}{% German
...
\fancyhead[L]{\sffamily\small\relscale{0.95}\strut\studentfullname: \textit{\manuscripttitle}}
\fancyhead[R]{\sffamily\small\relscale{0.95}\strut\thepage}
...
\newcommand*{\maketitleEN}{%
	...
	\begin{center}
		...
		\vfill\vfill
		{\huge\ifthenelse{\boolean{manuscriptisbachelor}}{Bachelor's Thesis}{%
			\ifthenelse{\boolean{manuscriptismaster}}{Master's Thesis}{Term Paper}%
		}\par}
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
			\vspace{1.75\baselineskip}
			Presented to the \\
			Department of Economics at the \\
			Rheinische Friedrich-Wilhelms-Universit\"at Bonn \par
		...
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
			...
			\ifthenelse{\equal{\manuscripttype}{BA-2F}}{%
				Bachelor of Arts (B.A.) \\
				in the double-major program \textit{\foreignlanguage{ngerman}{Wirtschaftswissenschaften}} and \secondmajor \\
				(\textit{\foreignlanguage{ngerman}{F\"acher im Zwei-Fach-Modell}})%
			}{%
				\ifthenelse{\equal{\manuscripttype}{BSc-ECS}}{%
					Bachelor of Science (B.Sc.) \\
					in Economics and Computer Science%
				}{%
					\ifthenelse{\equal{\manuscripttype}{BSc-VWL}}{%
						Bachelor of Science (B.Sc.) \\
						in \textit{\foreignlanguage{ngerman}{Volkswirtschaftslehre}}%
					}{%
						\ifthenelse{\boolean{manuscriptisbachelor}}{%
							Bachelor of Science (B.Sc.) \\
							in Economics%
						}{%
		...
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{Supervisor}{Instructor}: \\
		\advisorname
		\vfill\vfill%\vfill
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
...
\newcommand*{\maketitleDE}{%
	...
	\begin{center}
		...
		{\fontsize{27.5pt}{32pt}\bfseries\manuscripttitle\par}
		\vfill\vfill
		{\huge\ifthenelse{\boolean{manuscriptisbachelor}}{Bachelorarbeit}{%
			\ifthenelse{\boolean{manuscriptismaster}}{Masterarbeit}{Hausarbeit}%
		}\par}
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
			\vspace{1.75\baselineskip}
			zur Erlangung des Grades \\[\baselineskip]
			\ifthenelse{\equal{\manuscripttype}{BA-2F}}{%
				Bachelor of Arts (B.\,A.) \\
				in den Teilstudieng\"angen Wirtschaftswissenschaften und \secondmajor \\
				(F\"acher im Zwei-Fach-Modell)%
			}{%
				\ifthenelse{\equal{\manuscripttype}{BSc-ECS}}{%
					Bachelor of Science (B.\,Sc.) \\
					im Studiengang \textit{\foreignlanguage{USenglish}{Economics and Computer Science}}%
				}{%
					\ifthenelse{\boolean{manuscriptisbachelor}}{%
						Bachelor of Science (B.\,Sc.) \\
						...
						Master of Science (M.\,Sc.) \\
						im Studiengang \textit{\foreignlanguage{USenglish}{Economics}}%
					}%
				}%
			}%
			\\[\baselineskip]
		...
		\par
		\vfill\vfill%\vspace{2\baselineskip}
		Themensteller\ifthenelse{\equal{\advisorgender}{f}}{in}{\ifthenelse{\equal{\advisorgender}{m}}{}{*in}}: \\
		\advisorname
		\vfill\vfill%\vfill
		\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
...
\titleformat{name = \subparagraph, numberless}[runin]{}{}{0pt}{\itshape#1}[]
...
\setlist[enumerate, 2]{label = \alph*), align = left, labelwidth = \parindent, labelsep = 0pt}
...
\UseTblrLibrary{counter}  % To prevent incorrect numbering of equations embedded in tblr environments
...
% Autoref names for theorem-like environments
...
\ifthenelse{
	...
	\providecommand*{\acknowledgmentautorefname}{Kenntnisnahme}
	\providecommand*{\algorithmautorefname}{Algorithmus}
	\providecommand*{\assertionautorefname}{Zusicherung}
	\providecommand*{\assumptionautorefname}{Annahme}
	\providecommand*{\caseautorefname}{Fall}
	\providecommand*{\claimautorefname}{Behauptung}
	\providecommand*{\conclusionautorefname}{Schlussfolgerung}
	\providecommand*{\conditionautorefname}{Bedingung}
	\providecommand*{\conjectureautorefname}{Vermutung}
	\providecommand*{\corollaryautorefname}{Korollar}
	\providecommand*{\criterionautorefname}{Kriterium}
	\providecommand*{\exampleautorefname}{Beispiel}
	\providecommand*{\exerciseautorefname}{\"Ubung}
	\providecommand*{\hypothesisautorefname}{Hypothese}
	\providecommand*{\notationautorefname}{Notation}
	\providecommand*{\noteautorefname}{Hinweis}
	\providecommand*{\propertyautorefname}{Eigenschaft}
	\providecommand*{\propositionautorefname}{Satz}
	\providecommand*{\questionautorefname}{Frage}
	\providecommand*{\remarkautorefname}{Anmerkung}
	\providecommand*{\resultautorefname}{Resultat}
	\providecommand*{\summaryautorefname}{Zusammenfassung}
}{% English
	\providecommand*{\acknowledgmentautorefname}{Acknowledgment}
	\providecommand*{\algorithmautorefname}{Algorithm}
	\providecommand*{\assertionautorefname}{Assertion}
	...
	\providecommand*{\criterionautorefname}{Criterion}
	...
	\providecommand*{\summaryautorefname}{Summary}
}
...
\usepackage[
	...
	dashed = false,
	doi = only, isbn = false,
	maxbibnames = 6,  % See https://www.chicagomanualofstyle.org/tools_citationguide/citation-guide-2.html
	minbibnames = 3,  % See https://www.chicagomanualofstyle.org/tools_citationguide/citation-guide-2.html
	maxcitenames = 2,  % See https://www.chicagomanualofstyle.org/tools_citationguide/citation-guide-2.html
	mincitenames = 1,
	natbib = true,
...
{\selectlanguage{USenglish}\beware{%
	{\small\relscale{1.2}For compiling this document without any changes, a~\emph{complete} (and up-to-date) TeX distribution is required. \par} \medskip
	Options include
	\href{https://tug.org/mactex/mactex-download.html}{MacTeX},
	\href{https://miktex.org/download}{MiKTeX},
	and \href{https://tug.org/texlive/acquire-netinstall.html}{TeX Live}. \mdseries For MiKTeX, please choose the ``Net Installer'' to install the \emph{complete} MiKTeX distribution, rather than the ``Basic Installer.'' \par \medskip
	\bfseries Compilation via \url{https://overleaf-en.astro.uni-bonn.de} works in general. \mdseries Use ``Shibboleth login'' to log in with your Uni~\caps{ID}.  Unfortunately, compiling the entire document may fail. Try compiling a shorted version (e.g., remove the appendices). \par \medskip
	\bfseries Compilation via \url{https://uni-bonn.sciebo.de/apps/overleaf_nextcloud/launcher/launch} also works in general. \mdseries Here, compiling the entire document from scratch may lead to a~time-out. Compiling the document again (and again) can help. So does compiling a~shortened version (which can then be extended step by step). \par \medskip
...
\begin{enumerate}[widest* = 10]
	...
	\item In which way(s) does answering the research question contribute to closing the knowledge gap?
	\item Which method(s) do you use to answer the research question?
	\item Why is (are) the chosen method(s) (more) suitable (than alternative methods)?
\end{enumerate}
...
\begin{enumerate}[resume, widest* = 10]
	\item What was the knowledge gap before the current study? [Similar to the introduction.]
	\item \emph{What did you find, and how does it contribute to closing the knowledge gap? [A]} (Which method\slash data did you use, and what are the results?---Recap and take-home message.)
	\item How do your results relate to (confirm\slash contradict\slash complement\slash extend) the results from previous and other contemporaneous studies?
	\item What part of the knowledge gap is still open? (Phrased a~bit more negatively: What are the limitations of your approach?)
	\item Next steps\slash avenues for future research: How could we go about closing the remaining knowledge gap (by removing the limitations of the approaches used so far)?\footnote{John Cochrane might do away with question~12. See his ``Writing Tips for Ph.D. Students,'' \url{https://www.fma.org/assets/docs/membercontent/writing_cochrane.pdf}.}
\end{enumerate}
...
Relating the ``RAP'' approach to the list of questions above, question~4 is the ``R,'' the answer to question~9 is the ``A,'' and the remaining questions spell out the ``P.''
...
\footnote{See \url{https://www.econ.uni-bonn.de/examinations/de/informationen/bachelor/bachelorarbeit} and \url{https://www.econ.uni-bonn.de/examinations/en/information/master-economics/master-thesis}.}
...
\begin{longtblr}{}
	...
	{\relscale{0.83}\fontfamily{plxMono-TLF}\selectfont IBM Plex Mono: o\,0\,O}
	\par
	\normalsize%
...
\begin{itemize}
	...
	\item The line spacing for headings is also \printlength{\baselinedist}, in combination with vertical space above and below as follows:
		...
		\begin{itemize}
			...
			\item Level-4 headings (\verb|\paragraph|) are spaced \makeatletter\SI[round-mode = places, round-precision = 2]{\strip@pt\baselinedistHalf}{pt}\makeatother\ above and 0~pt below. (A~fourth sectioning level is usually too much for a~thesis.)
		\end{itemize}
...
\subsubsection{Paraphrasing Other Authors' Ideas}

The following is an example of an in-text citation that introduces a~paraphrase of other authors' ideas:
\begin{longtblr}{}
	\cite{sarfraz} suggest an algorithm to automatically capture the outlines of fonts.
\end{longtblr}
\noindent The author's name or authors' names, respectively, can be embedded in the sentence, as in the example above, or surrounded by parentheses:
\begin{longtblr}{}
...
Citing a~work with three authors or more via \verb|\cite|, \verb|\citep|, \verb|\citet|, etc. abbreviates the author list to first author plus ``et~al.'' by default:
\begin{longtblr}{}
...
To cite particular pieces of information of a~work, several commands are available, such as \verb|\citeauthor|, \verb|\citeyear|, \verb|\citetitle|, \verb|\citetitle*|, etc.:
\begin{longtblr}{}
...
\begin{longtblr}{}
	...
	\begin{minipage}{\linewidth}\noindent\strut%
		Variable $d$ vs.\ differential $\dd$, variable $\Delta$ vs.\ difference operator $\upDelta$, limit $\lim$ vs.\ product $l\,i\,m$.
		\begin{equation}
...
The typeface (Times), font sizes, and line spacing in this template were chosen such that a~virtually identical layout can be achieved with other typesetting software such as Microsoft Word, Apple Pages, and the open-source alternatives Libre\-Office Writer (\url{https://www.libreoffice.org}) and Typst (\url{https://typst.app}). This means taking into which fonts are available across applications and operating systems, and the fact that Word restricts font sizes to multiples of 0.5~pt.

\subsection{Clean Code/Semantic Coding: Separating Content and Formatting}

Beyond words, sentences, and paragraphs, academic texts are structured via sections, subsections, and often subsubsections. Moreover, academic texts often feature figures, tables, lists of references, and potentially appendices. These structural elements are associated with headings\slash titles and captions.
...
\subsubsection{Combining \mbox{\textit{tabularray}} with the \textit{siunitx} Package}
...
\renewcommand{\testlist}{%
	...
	\item We delve into the various environments available in LaTeX, such as the \texttt{equation}, \texttt{figure}, \texttt{table}, \texttt{enumerate}, and \texttt{itemize} environments.
...
\begin{description}
	...
	\item[Cherry.] A~small, round fruit that is typically red or dark purple in color and contains a~hard pit in the center. Cherries are rich in antioxidants and anti-inflammatory compounds.\footnote{These descriptions were \caps{LLM}-generated.}
\end{description}
...
\ifthenelse{% Test translation proof -> Beweis
	\equal{\manuscriptlanguage}{DE}%
}{% German
...
\uppi \, \Re[w(x + \mathrm{i}\,y)], \quad w(z) = \mathrm{e}^{-z^2} \operatorname{erfc}(-\mathrm{i}\, z), \quad x \in \mathbb{R}, y>0.
\end{equation}
...
\subparagraph{No control.} Even the all-powerful Pointing has no control about the blind texts it is an almost unorthographic life
...
\begin{proof}[Proof of \autoref{lemma:budde-goex-1}]
	Since expected profit is linear in $k_i$, the firm chooses the maximum capacity level ${k = 1}$ when \eqref{eq:opt-capacity} is positive, and the minimum capacity ${k_i = 0}$ when \eqref{eq:opt-capacity} is negative. Taking into account that in equilibrium, the condition ${B^{-1}(b_i^*) = c_i}$ must hold for all~$i$, the critical value~$\hat{c}$ for the indifferent bidder is obtained by solving~\eqref{eq:opt-capacity} for~$c_i$.
\end{proof}
...
\iftrue
	...
	\subsection[Font Sample Times]{\fontfamily{ntxtlf}Font Sample \textit{Times}}
	...
	\subsection[Font Sample Palatino]{\fontfamily{zpltlf}\selectfont Font Sample \textit{Palatino}}
	...
	\subsection[Font Sample Utopia]{\fontfamily{erewhon-TLF}\selectfont Font Sample \textit{Utopia}}
	...
	\subsection[Font Sample Charter]{\fontfamily{XCharter-TLF}\selectfont Font Sample \textit{Charter}}
	...
	\subsection[Font Sample {\textls[40]{STIX}} Two]{\fontfamily{SticksTooText-TLF}\selectfont Font Sample \textit{\textls[40]{STIX} Two}}
	...
	\subsection[Font Sample Libertinus Serif]{\fontfamily{LibertinusSerif-TLF}\selectfont Font Sample \textit{Libertinus Serif}}
	...
	\subsection[Font Sample New Century Schoolbook]{\fontfamily{TeXGyreScholaX-TLF}\selectfont{\relscale{0.925}Font Sample \textit{New Century Schoolbook}}}
...
\ifthenelse{\boolean{manuscriptisbachelor} \OR \boolean{manuscriptismaster}}{%
	...
	\ifthenelse{%
		...
		I~hereby declare that I~have written the present thesis independently and have used no other sources and aids than those indicated, that the submitted work has not been previously presented for examination at another university, and that it has not been published either in whole or in part. I~have identified all direct quotes and paraphrased ideas taken from other works in each individual case. \par
		\vspace{\baselineskip}
